...
public class TransactionServiceImpl implements TransactionService {
	...
	private Mono<Boolean> verifyAccountNumber(String accountNumber) {
		return webClient.get()
				...
				.doOnError(error -> {
					// Manejo de errores
					System.err.println("Error al verificar el número de cuenta: " + error.getMessage());
				});
	}
	...
	private Mono<Void> updateAccountBalance(String accountNumber, Double amount) {
		return webClient.patch()
				...
				.doOnError(error -> {
					System.err.println("Error al actualizar el balance: " + error.getMessage());
				});
	}
	...
	private Mono<Double> getAccountBalance(String accountNumber) {
		return webClient.get()
				...
				.doOnError(error -> {
					System.err.println("Error al obtener el balance: " + error.getMessage());
				});
	}
		...
		return verifyAccountNumber(transfer.destinationAccount())
				.flatMap(destinationAccountExists -> {
					if (!destinationAccountExists) {
						return Mono.error(new IllegalArgumentException("Destination account does not exist"));
					}
